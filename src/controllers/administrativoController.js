...
// GET /administrativo/patrimonio/dashboard
...
// GET /administrativo/patrimonio/ativos
...
// GET /administrativo/patrimonio/ativos/novo
...
// POST /administrativo/patrimonio/ativos
const createAtivo = async (req, res) => {
  try {
    ...
    res.redirect('/administrativo/patrimonio/ativos?success=created');
  } catch (error) {
    res.render('administrativo/patrimonio/ativos/form', {
      title: 'Novo Ativo',
      user: req.user,
...
// GET /administrativo/patrimonio/ativos/:id
const showAtivo = async (req, res) => {
  try {
    ...
    res.redirect('/administrativo/patrimonio/ativos?error=' + encodeURIComponent(error.message));
  }
};

// Função para exibir formulário de edição de ativo
// GET /administrativo/patrimonio/ativos/:id/editar
...
// POST /administrativo/patrimonio/ativos/:id
const updateAtivo = async (req, res) => {
  try {
    ...
    res.redirect('/administrativo/patrimonio/ativos/' + req.params.id + '?success=updated');
  } catch (error) {
    try {
      const asset = await patrimonioService.getAssetById(req.params.id, req.user.condominiumId);
...
// GET /administrativo/patrimonio/ativos/:id/manutencao/nova
...
// POST /administrativo/patrimonio/ativos/:id/manutencao
const createManutencao = async (req, res) => {
  try {
    ...
    res.redirect('/administrativo/patrimonio/ativos/' + req.params.id + '?success=maintenance_created');
  } catch (error) {
    try {
      const asset = await patrimonioService.getAssetById(req.params.id, req.user.condominiumId);
...
// POST /administrativo/patrimonio/ativos/:id/calcular-depreciacao
const calculateDepreciation = async (req, res) => {
  try {
    ...
    res.redirect('/administrativo/patrimonio/ativos/' + req.params.id + '?success=depreciation_calculated');
  } catch (error) {
    console.error('Erro ao calcular depreciação:', error);
    res.redirect('/administrativo/patrimonio/ativos/' + req.params.id + '?error=' + encodeURIComponent(error.message));
  }
};

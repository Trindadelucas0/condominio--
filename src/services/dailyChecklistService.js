  ...
  try {
    ...
    for (const model of models) {
      ...
      if (existsResult.rows.length === 0) {
        // Busca primeiro usuário do departamento (role) para atribuir
        let assignedTo = null;
        if (model.default_assigned_role) {
          const userResult = await query(
            `SELECT u.id FROM users u
             ...
             AND u.active = TRUE
             LIMIT 1`,
            [condominiumId, model.default_assigned_role]
          );
        ...
        const checklistResult = await query(
          ...
          [
            ...
            dateStr,
            assignedTo,
            model.default_assigned_role
          ]
        );

        const checklist = checklistResult.rows[0];

        // Busca itens do modelo e cria itens do checklist
...
const updateChecklistItem = async (itemId, status, comment, userId, condominiumId, ipAddress, userAgent) => {
  try {
    ...
    const itemResult = await query(
      `SELECT dci.*, dc.id as checklist_id, dc.condominium_id,
    ...
    await logAction({
      ...
      entityId: itemId,
      beforeData: item,
      afterData: updatedItemResult.rows[0],
      ipAddress: ipAddress,
      userAgent: userAgent
    });

...
const authenticate = async (req, res, next) => {
  try {
    // Busca token JWT no cookie (tenta accessToken primeiro, depois token para compatibilidade)
    let token = req.cookies.accessToken || req.cookies.token || null;

    // Se não há access token, tenta renovar com refresh token
    if (!token) {
      ...
      if (refreshToken) {
        try {
          ...
          return res.status(401).redirect('/auth/login?error=token_expired');
        }
      } else {
    ...
    try {
      ...
      // Se access token expirou, tenta refresh token
      const refreshToken = req.cookies.refreshToken;
      if (refreshToken) {
        try {
          const authService = require('../services/authService');
          ...
          decoded = jwtHelper.verifyAccessToken(refreshed.accessToken);
        } catch (refreshError) {
          res.clearCookie('accessToken');
          res.clearCookie('refreshToken');
          res.clearCookie('token');
